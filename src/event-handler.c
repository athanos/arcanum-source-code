...
EVENT_DATA *alloc_event()
{
  ...
  event->fun        = NULL;
  event->argument   = NULL;
  event->owner.ch = NULL;  /* only need to NULL one of the union members */
  event->passes     = 0;
  event->bucket     = 0;
...
/* function   :: add_event_char()
 * arguments  :: the event, the owner and the delay
 * ======================================================
 * This function attaches an event to a char, and sets
...
void add_event_char(EVENT_DATA *event, CHAR_DATA *ch, int delay)
{
  ...
  if (event->fun == NULL)
  {
    bug("add_event_char: event type %d has no callback function.", event->type);
    return;
  }
  ...
  /* attach the event to the chars local list */
  AttachToList(event, ch->events);

  /* attempt to enqueue the event */
  if (enqueue_event(event, delay) == FALSE)
    bug("add_event_char: event type %d failed to be enqueued.", event->type);
}
...
/* function   :: event_isset_char()
 * arguments  :: the char and the type of event
 * ======================================================
 * This function checks to see if a given type of event
 * is enqueued/attached to a given char, and if it is,
 * it will return a pointer to this event.
 */
